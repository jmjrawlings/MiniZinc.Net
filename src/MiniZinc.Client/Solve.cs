...
public class Solve
{
    ...
    internal Solve(MiniZincClient minizinc, Solver solver, SyntaxTree model, ILogger? logger = null)
    {
    ...
    /// </summary>
    /// <param name="token"></param>
    public async IAsyncEnumerable<Solution> Solutions(CancellationToken token = default)
    {
        var solveStart = DateTimeOffset.Now;
        var iterStart = solveStart;
        var iteration = 0;
