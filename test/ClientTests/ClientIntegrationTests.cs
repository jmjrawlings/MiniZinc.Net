...
public class ClientIntegrationTests : IClassFixture<ClientFixture> {
	...
	public async Task test_solve_unit_compilation_time_limit() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().Be("time limit reached");
	}
	...
	public async Task test_solve_unit_general_array_access_out_of_bounds_1() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*array access out of bounds, array `x' has index set A..B, but given index is C.*");
	}
	...
	public async Task test_solve_unit_general_array_access_out_of_bounds_2() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*array access out of bounds, array `x' is empty.*");
	}
	...
	public async Task test_solve_unit_general_array_access_record_out_of_bounds() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*array access out of bounds.*");
	}
	...
	public async Task test_solve_unit_general_enum_out_of_range_1() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `x' is {A}, but assigned value is B.*");
	}
	...
	public async Task test_solve_unit_general_function_param_out_of_range() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `'<unnamed argument>'' is 1..1, but assigned value is 2.*");
	}
	...
	public async Task test_solve_unit_general_function_return_out_of_range() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*result of function `foo' is B, which violates function type-inst {A}.*");
	}
	...
	public async Task test_solve_unit_general_function_return_out_of_range_opt() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*function result violates function type-inst, array contains value B which is not contained in {A}.*");
	}
	...
	public async Task test_solve_unit_general_param_out_of_range_float() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `x' is -infinity..-1.0 union 1.0..infinity, but assigned value is 0.0.*");
	}
	...
	public async Task test_solve_unit_general_param_out_of_range_int() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `x' is -infinity..-1 union 1..infinity, but assigned value is 0.*");
	}
	...
	public async Task test_solve_unit_general_stack_overflow() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*stack overflow.*");
	}
	...
	public async Task test_solve_unit_regression_github_648_par_array_decl() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `x[1]' is 1..3, but assigned value is 0.*");
	}
	...
	public async Task test_solve_unit_regression_github_648_par_decl() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*declared domain of `x' is 1..3, but assigned value is 0.*");
	}
	...
	public async Task test_solve_unit_regression_github_656() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*cannot coerce set into 2-dimensional array.*");
	}
	...
	public async Task test_solve_unit_regression_ti_error_location() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex("^(?!unknown file:0.0).*$");
	}
	...
	public async Task test_solve_unit_types_nested_type_inst_id() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*type-inst variable $X used in both array and non-array position.*");
	}
	...
	public async Task test_solve_unit_types_struct_index_sets_1() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".* Declared index set of `x.1' is [1..1], but is assigned to array with index set [2..2].*");
	}
	...
	public async Task test_solve_unit_types_struct_return_ti_3() {
		...
		var solution = await MiniZinc.Solve(model, options);
		solution.Status.Should().Be(SolveStatus.Error);
		solution.Output.Should().MatchRegex(".*function result violates function type-inst.*");
	}
