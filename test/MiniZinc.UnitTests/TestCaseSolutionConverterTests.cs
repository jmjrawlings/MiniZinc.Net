...
public sealed class TestCaseSolutionConverterTests
{
    private static TestCaseSolution Sol(params (string, TestCaseSolutionValue)[] vars)
    {
        Dictionary<string, TestCaseSolutionValue> d = new();
        foreach ((string name, TestCaseSolutionValue val) in vars)
            d[name] = val;
        return new TestCaseSolution(d);
    }

    [Fact]
    public void converts_scalars()
    {
        MiniZincData data = TestCaseSolutionConverter.FromSolution(
    ...
    public void round_trips_through_comparer()
    {
        ...
        MiniZincData expected = TestCaseSolutionConverter.FromSolution(
        ...
        Parser.TryParseDataString("x = 42;\na = [1, 2];", out MiniZincData? actual, out _, out _, out _)
            .ShouldBeTrue();
        MiniZincDataComparer.Compare(expected, actual!).IsMatch.ShouldBeTrue();
    }
        ...
        MiniZincData data = TestCaseSolutionConverter.FromSolution(
            Sol(
                (
                    "r",
                    new RecordVal(
                        new Dictionary<string, TestCaseSolutionValue> { ["a"] = new IntVal(1), ["b"] = new IntVal(2) }
                    )
                )
            )
        );
    ...
    public void try_from_solution_reports_unrenderable()
    {
        ...
        bool ok = TestCaseSolutionConverter.TryFromSolution(Sol(), out _, out string? error);
        ok.ShouldBeFalse();
